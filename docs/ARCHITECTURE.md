...
| Agent-tool bridge | **Validate** | Unchanged: no Factoru tool has reached a real agent harness. Gas City 1.4.0 does expose `gc mcp list` projection per agent/session, so the mechanism exists and has not been tried. | Complete one authenticated, role-scoped probe-tool round trip through both Claude and Codex before the pack contract stabilizes. |
