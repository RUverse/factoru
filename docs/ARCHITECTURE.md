...
| Agent-tool bridge | **Validate** | The transport is identified: a pack-level `mcp/` entry projects to `<workdir>/.codex/config.toml` for Codex and `<workdir>/.mcp.json` for Claude, verified for both. (Only the per-agent `mcp` arrays are inert tombstones.) A token-scoped probe server exists. No agent has called it yet. | Complete one authenticated, role-scoped probe-tool round trip through both harnesses before the pack contract stabilizes. |
