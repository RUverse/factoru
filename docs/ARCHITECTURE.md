...
| Agent-tool bridge | **Validate — attempted and failed** | A pack `mcp/` entry and a token-scoped probe server exist, and `gc mcp list` reports the projection target for both harnesses (`.codex/config.toml`, `.mcp.json`). But a real agent asked to call the tool reported it was not exposed, and no harness config was written. `gc mcp list` describes a *planned* projection, not a materialised one. | Find what performs the `stage1` delivery — provider `session_setup`, a session-provider script, or a Factoru-side bridge — then complete one round trip through both harnesses. |
...
| `packages/gas-city` | **Partial** | Factoru-owned orchestration port over Gas City 1.4.0. Compatibility pinning, readiness, supervisor client, event cursors, rig-registration safety, and run dispatch/observation exist; conversation delivery, session lifecycle, and recovery do not. |
