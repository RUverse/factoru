...
> Runtime implementation status: Milestones 5 and 6 application paths are
> connected; provider-backed operator acceptance remains
...
| Factoru Server | **Partial** | Fastify connects SQLite-backed remote pairing and private loopback enrollment, trusted devices, authenticated one-time WebSocket tickets, scoped live methods, durable projects/workspaces/tasks, idempotent product commands, event/outbox reactors, Project Manager conversation delivery, Queue reconciliation, and restart observation while remaining loopback-bound. The serial delivery reactor admits one ready task, prepares/adopts its capsule, dispatches and observes `software-delivery`, collects evidence and usage, performs integration checks, and exposes idempotent run decisions. | Complete the provider-backed ten-run Formula acceptance and one whole production-loop restart drill. |
| Shared protocol | **Implemented** | `packages/protocol` owns runtime-validated health/handshake, pairing/enrollment, ticket, project/repository/device/workspace/conversation/Worker-Type/task/Queue/run-evidence, live request/response/event, cursor snapshot, compatibility, and typed HTTP client schemas. Run decisions are explicit named methods; older cached workspaces receive safe empty collection defaults. | Extend only when a later milestone owns a new wire capability. |
| Factoru Desktop | **Partial** | Electron main persists server-ID-bound profiles, encrypted credentials, projects, selected workspace, conversations, Workers, tasks, task runs, and cursors; it owns the authenticated live connection and exposes named IPC. The renderer adds a four-state board with run stage, raw steps, logs, checks, cost, failures, review evidence, and cancel/retry/request-changes/approve/archive controls. | Complete the real single-task desktop acceptance; managed launch and packaged Mac acceptance remain later milestones. |
| Gas City adapter | **Partial** | `packages/gas-city` is verified against Gas City 1.4.0: compatibility/readiness, loopback supervisor client, durable cursors, guarded rig registration, run dispatch/observation/cancellation, per-run usage folding, and project-runtime configuration. Dispatch reads and strictly validates the Factoru Formula source before any durable Gas City mutation. Raw DTOs and provider options stay inside the package. | Complete the provider-backed ten-run/restart acceptance matrix. |
...
| Factoru Gas City pack | **Partial** | `packs/factoru-default` 0.3.0 defines four provider-neutral roles, prior probes, Queue reconciliation, the bounded production `software-delivery` Formula v2, and the role-scoped MCP bridge. Delivery routes implementation and independent review to separate bindings, uses real `needs` edges, a trusted deterministic check with two total attempts, and a final evidence step. | Complete the ten-run operational acceptance and tune only from measured failures. |
| Factoru database | **Implemented** | `@factoru/database` uses `better-sqlite3`, WAL/foreign keys/busy handling, forward-only SQL migrations, identity binding, transactional receipts/events/outbox, checkpoint, and online backup. Migration 0004 adds execution stage, formula hash/version, capsule/branch identity, steps, logs, usage, review package, error, archive, and one-active-run indexes. Admission, observation, failure/retry, cancellation, review handoff, and acceptance are transactional. | Exercise reopen/recovery with real Gas City correlations. |
...
| Internal review | **Partial** | The production formula routes review to a distinct agent binding and persists its report; capsule finalization includes it in the review package. | Prove useful findings across the ten real tasks. |
| Human review | **Partial** | Completed runs move to `needs_you` with a diff/check/review/usage package and explicit approve/request-changes/archive controls. | Time and accept a real provider-produced review package. |
| Task-run capsule | **Partial** | Factoru Server owns one deterministic branch/worktree and external control/evidence directory per run, passes the same capsule to implementer/reviewer, safely adopts it after restart, rebases, reruns trusted checks, and refuses dirty/conflicted/invalid capsules. | Complete the real run and failure/restart acceptance matrix without repository damage. |
...
| --- | --- | --- |
| `apps/desktop` | **Partial** | Electron main owns profiles, encrypted credentials, private local-enrollment access, authenticated live transport, workspace/run cache and cursors, and named IPC. The renderer implements onboarding, project/PM/Workers surfaces, the four-state board, and progressively disclosed delivery stages, steps, logs, cost, checks, failures, review evidence, and decisions. Packaged local install remains later. |
| `apps/server` | **Partial** | Fastify serves health/auth/live methods, project/workspace/task services, idempotent commands, outbox/reactors, the loopback agent-tool gateway, Queue planning, and the serial execution loop. The server owns capsule creation/adoption, trusted checks, final integration validation, review packaging, and decision transitions. |
...
| `packages/database` | **Implemented** | SQLite connection policy, forward migrations, transactional event/outbox writes, backup/reopen recovery, product/task persistence, and full serial execution evidence plus transition persistence through migration 0004. |
| `packages/gas-city` | **Partial** | Factoru-owned orchestration port over Gas City 1.4.0. In addition to compatibility, cursors, registration, conversation, and configuration, it strictly validates Formula v2 semantics/typed variables before dispatch, observes/cancels runs, and folds per-run worker-operation usage. Live provider acceptance remains. |
...
| `packs/factoru-default` | **Partial** | Version 0.3.0 contains four agent-role contracts, prior probes, role-scoped task tools, Queue reconciliation, and the bounded `software-delivery` Formula v2; both production formulas lint under Gas City 1.4.0. Provider-backed acceptance remains. |
...
**Partial tier one; later tiers deferred.** A capsule is one Factoru-owned
...
| Formula semantic validator | **Partial** | Production dispatch parses the exact Formula source, requires compiler `>=2.0.0`, enforces declared string/int/bool variables, rejects unknown inputs plus inert gate/waits-for/until, deprecated contract/on-complete fan-out, v2 converge, and uncapped drains above the Factoru ceiling. The real ten-run gate must still prove the accepted subset. |
