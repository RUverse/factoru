...
async function main() {
  ...
  const projectsRoot = path.join(os.homedir(), 'factoru-projects')
  await mkdir(projectsRoot, { recursive: true, mode: 0o700 })
  const projectsStats = await stat(projectsRoot)
  if (!projectsStats.isDirectory()) throw new Error(`${projectsRoot} is not a directory`)
  ...
  process.stdout.write(`Managed projects: ${projectsRoot}\n`)
  process.stdout.write(`Repository import locations: ${repositoriesRoot}\n`)
  process.stdout.write(`Operator CLI: ${cliPath}\n`)
  process.stdout.write(`Current shell: export PATH=${JSON.stringify(binDirectory)}:"$PATH"\n`)
